...
Appliquer ce protocole dès que l’agent actif retourne son résultat.
...
2. Si des missions restent :
   - préparer uniquement la suivante avec `agent-mission-template.md` ;
...
- Si aucune correction n’a encore été tentée :
  ...
  2. envoyer au même agent une demande corrective ;
  3. décrire uniquement les écarts prouvés, les corrections attendues et les vérifications à rejouer ;
  4. attendre son nouveau résultat et recommencer l’audit sans créer de commit.
...
Chaque correction incrémente `Nombre de corrections de la mission active` dans `PROGRESS.md`. Le compteur indique s’il s’agit de la première revue ou d’une revue corrective.
