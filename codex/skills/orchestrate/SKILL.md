---
name: orchestrate
description: Piloter un besoin de sa conception à sa réalisation, directement ou au moyen d’agents Codex séquentiels.
---

Reste responsable du résultat global, de la conception à la réalisation. L’invocation de ce skill constitue une demande explicite de délégation à des agents Codex. Par défaut, ne modifie pas le code : inspecte le dépôt, pilote les agents, audite leurs résultats, maintiens le contexte d’orchestration et crée les commits des missions acceptées. L’exécution directe constitue l’unique exception.
...
6. Sinon, utilise le workflow par agents Codex.

### Orchestration existante

Résume l’état de `PROGRESS.md`, vérifie qu’il correspond au dépôt, puis reprends à la mission active ou à la prochaine action. Si un agent actif est encore disponible, continue à le piloter. Sinon, audite d’abord les changements présents avant de créer un nouvel agent borné. Ne recommence pas la conception sauf si le plan est explicitement invalidé.

### Résultat d’un agent

Quand l’agent actif termine, attends son résultat puis lis `agent-review.md` et applique intégralement son protocole. La fin de l’agent déclenche toujours un audit ; ce n’est jamais une acceptation automatique.
...
3. Inscris dans `PROGRESS.md` la mission active, l’agent prévu, ses critères d’acceptation et l’état de la correction.
4. Crée un agent Codex borné à cette mission.
5. Garde un seul agent de mission actif, attends son résultat et audite-le immédiatement avec `agent-review.md`.
6. Ne délègue la mission suivante qu’après acceptation et commit de la mission courante.

Les missions sont toujours séquentielles. L’orchestrateur crée, pilote et attend les agents sans transfert manuel par l’utilisateur.
...
- Ne crée jamais plus d’un agent de mission actif.
