...
Assert-Contains $skill 'contexte delta' 'orchestrate must avoid copying the global context into mission prompts'
Assert-Contains $skill 'check:release' 'orchestrate must reserve release verification for final integration'
Assert-Contains $skill 'agent-mission-template.md' 'orchestrate must keep the mission report contract in one source of truth'
Assert-Contains $skill 'manifeste de preuve' 'orchestrate must use verification evidence manifests'
...
Assert-Contains $missionTemplate 'profil de verification' 'missions must declare the verification profile'
Assert-Contains $missionTemplate 'STATUS: accepted | rejected | blocked' 'mission reports must use the compact report contract'
Assert-Contains $missionTemplate 'blocage environnemental' 'missions must stop on classified environment blockers'
...
Assert-Contains $review 'Ne pas rejouer une preuve verte' 'review must inspect evidence before replaying checks'
